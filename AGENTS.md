...
Commits follow short, imperative titles in Spanish or English (`mejora importación archivos .docx`, `Fix: Increase server action body size limit`) and stay scoped to one concern. PRs must include a summary, testing evidence (`npm run lint`, manual flows), screenshots or GIFs for UI work, and confirmation that roadmap/checklist docs (START*HERE\*, MVP*\*) are updated or unaffected. Do not merge until reviewers validate Prisma changes and environment updates.
