/**
 * Content Paginator - FIXED VERSION
 * Splits HTML content into pages based on device dimensions
 *
 * FIXES:
 * 1. More accurate line estimation for paragraphs
 * 2. Better handling of spacing between elements
 * 3. Reduced lines per page to account for visual spacing
 */
...
export function paginateContent(
  ...
  // FIXED: Apply 0.75 factor to account for visual spacing, margins between elements, etc.
  ...
  for (const node of nodes) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      if (element.tagName === "H1" || element.tagName === "H2") {
        ...
        // FIXED: Force page break before major headings (except at page start)
 ...
 * FIXED: More accurate estimations with spacing
 */
function estimateNodeLines(node: Node, config: PaginationConfig): number {
  if (node.nodeType === Node.TEXT_NODE) {
    ...
    // FIXED: More accurate character width estimate (0.5 instead of 0.6)
  ...
  if (node.nodeType === Node.ELEMENT_NODE) {
    ...
    // Headings take more space - FIXED: increased spacing
    ...
    // Images - FIXED: more realistic image size
    if (tagName === "IMG") {
      return 15; // Images take ~15 lines
    }

    // Lists - FIXED: better spacing between items
    ...
    // Paragraphs - FIXED: account for paragraph margins
    if (tagName === "P") {
    ...
    // Blockquotes - FIXED: increased indent and spacing
...
export function paginateContentWithMeasurement(
  ...
  let currentHeight = 0;
  const availableHeight =
    (config.pageHeight - config.marginTop - config.marginBottom) * 0.9; // 90% to be safe
  let currentChapter = "";
