...
async function runTests() {
  ...
  const tests = [
    '/home/ubuntu/upload/AnálisisdeFanfic_ThrillerPsicológicoyFantasía.docx',
    '/home/ubuntu/upload/Blog_GLM-4-6V.pdf',
  ];
  ...
  for (const testPath of tests) {
    if (fs.existsSync(testPath)) {
      const result = await testImport(testPath);
      if (result) passed++;
      else failed++;
    } else {
      console.log(`\n⚠️  Archivo no encontrado: ${testPath}`);
      failed++;
    }
  }
